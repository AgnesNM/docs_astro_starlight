---
title: Props
description: Learn about Props in Vue.js
---

This page assumes you've already read the [Components Basics](https://vuejs.org/guide/components/basics). Read that first if you are new to components.
...
### In Single-File Components Using `<script setup>`
...
- If using [Type-based props declarations](#type-based-props-declarations-ts), Vue will try its best to compile the type annotations into equivalent runtime prop declarations. For example, `defineProps<{ msg: string }>` will be compiled into `{ msg: { type: String, required: true } }`.
