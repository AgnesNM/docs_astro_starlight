---
title: Events
description: Learn about Events in Vue.js
---

This page assumes you've already read the [Components Basics](https://vuejs.org/guide/components/basics.html). Read that first if you are new to components.
...
The `emits` option and `defineEmits()` macro also support an object syntax. If using TypeScript with `<script setup>`, you can also perform type checking for the payload of the emitted events:
...
When using TypeScript with `<script setup>`, it's also possible to declare emitted events using pure type annotations:
