...
<Aside type="note" title="Prerequisites">- Familiarity with the command line - Install [Node.js](https://nodejs.org/en) version 18.3 or higher </Aside>
...
Make sure you have an up-to-date version of [Node.js] (https://nodejs.org/en) installed and your current working directory is the one where you intend
to create a project. Run the following command in your command line (without the $ sign):
...
<Tabs>
  ...
  <TabItem label="yarn"> ```sh yarn create vue@latest```</TabItem>
  <TabItem label="bun">```sh bun create vue@latest```</TabItem>
</Tabs>
...
If you are unsure about an option, simply choose `No` by hitting enter for now. Once the project is created, follow the instructions to install 
...
- More tooling details, including integration with backend frameworks, are discussed in the [Tooling Guide](/vue/06_scaling-up/tooling)
...
[CodePen Demo](https://vuejs.org/examples/#hello-world)

<Aside type="tip" title="TIP">Many of the examples for Composition API throughout the guide will be using the `<script setup>` syntax, which requires build tools. If you intend to use Composition API without a build step, consult the usage of the `setup()` [option](https://vuejs.org/api/composition-api-setup.html).</Aside>
...
<Aside type="tip" title = "Import Maps Browser Support"> Import Maps is a relatively new browser feature. Make sure to use a browser within its [support range](https://caniuse.com/?search=importmap). In particular, it is only supported in Safari 16.4+.</Aside>

<Aside type="note" title="Notes on Production Use">The examples so far are using the development build of Vue - if you intend to use Vue from a CDN in production, make sure to check out the [Production Deployment Guide](/vue/07_best-practices/tooling). 
While it is possible to use Vue without a build system, an alternative approach to consider is using `vuejs/petite-vue` that could better suit the context where `jquery/jquery` (in the past) or `alpinejs/alpine` (in the present) might be used instead.</Aside>
...
<CardGrid>
  <Card title="Continue with the Guide" href="/vue/02_essentials/creating-an-application.mdx">
    The guide walks you through every aspect of the framework in full detail.
  </Card>
  <Card title="Try the Tutorial" href="https://vuejs.org/tutorial/#step-1">
    For those who prefer learning things hands-on.
  </Card>
  <Card title="Check out the Examples" href="https://vuejs.org/examples/#hello-world">
    Explore examples of core features and common UI tasks.
  </Card>
</CardGrid>

<Aside type="tip">[Edit this page on GitHub](https://github.com/vuejs/docs/edit/main/src/guide/quick-start.md)</Aside>