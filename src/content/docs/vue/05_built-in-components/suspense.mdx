---
title: Suspense
description: Learn about Suspense in Vue.js
---
...
It is common to want to use `<Suspense>` in combination with the `<Transition>`, `<KeepAlive>`, and `<RouterView>` components from Vue Router.

The following example shows how to nest these components so that they all work correctly:
