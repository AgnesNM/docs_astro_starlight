---
title: CompositionApiFaq
description: Learn about CompositionApiFaq in Vue.js
---
...
Composition API is a built-in feature of Vue 3 and Vue 2.7. For older Vue 2 versions, use the officially maintained `@vue/composition-api` plugin. In Vue 3, it is also available via the `<script setup>` syntax.
...
The primary advantage of Composition API is that it enables clean, efficient logic reuse in the form of Composable functions. It solves all the drawbacks of mixins, Composition API's logic reuse capability has given rise to impressive community projects such as VueUse, an ever-growing collection of composable utilities. It also serves as a clean mechanism for easily integrating stateful third-party services or libraries into Vue's reactivity system, for example immutable data, state machines, and RxJS.
...
[Image: Options API code organization visual]
...
[Image: Options API vs Composition API code organization visual]
...
We do not recommend using Class API with Vue 3, as Composition API provides superior TypeScript integration and logic reuse benefits.
